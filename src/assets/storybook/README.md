...
素材通过 Vite 模块 URL 引用，构建后具有内容哈希，支持相对路径部署。连续地形由 `src/ui/board/terrainArt.js` 绘制，主体/功能标记由 `storybookArt.js` 绘制，角色状态由 `src/ui/game/unitRenderer.js` 组合。原始 PNG 不进入正式构建。
